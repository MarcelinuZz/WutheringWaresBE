...
const ITEM_SERVICE_URL = process.env.ITEM_SERVICE_URL;
...
export const requestItemService = async (endpoint, method = 'GET') => {
    const response = await fetch(`${ITEM_SERVICE_URL}${endpoint}`, {
        method,
    ...
    if (!response.ok) {
        const error = new Error(data.message || 'Item service error');
        error.status = response.status;
        throw error;
    }
